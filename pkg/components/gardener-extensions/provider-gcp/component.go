...
const (
	...
	ComponentDirectory = "gardener-extensions/provider-gcp"
)
...
func writeBaseTemplateFiles(opts components.Options) error {
	version, exists := opts.GetComponentVector().FindComponentVersion(componentvector.NameGardenerGardenerExtensionProviderGcp)
	if !exists {
		opts.GetLogger().Info("Component version not found in component vector, falling back to empty version", "component", componentvector.NameGardenerGardenerExtensionProviderGcp)
	}
