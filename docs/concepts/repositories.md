...
In the example above, the landscape repository is checked out at `/path/to/landscape-repo`, the landscape configuration sits at the repository root (`target: ./`), and the base repository is mounted within the landscape repo at `./base` (`baseLink: ./base`) — either as a Git submodule or as an in-tree subdirectory, depending on the chosen [organization](#organization). `base.target: ./` declares that the base content lives at the root of the base repository, so `path.Join(baseLink, base.target) = ./base` — the mount point already *is* the base content directory.
