...
```bash
glk generate --base-dir /path/to/base-repo
```

Whereas the generation of the landscape repositories requires the `--base-dir` flag to contain a relative path, seen from the landscape repo.
In the following example, the base repository is located in a `base` subdirectory of the landscape repository (either through Git submodule or Flux `include`):

```bash
landscape_dir=/path/to/landscape-repo
glk generate --base-dir ${landscape_dir}/base --landscape-dir $landscape_dir
```
